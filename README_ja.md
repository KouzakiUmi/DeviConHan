...
1. ゲームを終了し、パッチ適用タブで同梱パッチまたはカスタム ZIP を選択して適用します。ツールボックス版では、先に開発者ツールの設定管理でパッチ適用を有効にし、有効化後に選択できる最初のタブで ZIP を指定してください。同梱版では既定で有効です。
...
**🌐 英語パッチ**: [EnglishPatch.zip](EnglishPatch.zip) は『でびるコネクショん』の英語パッチです。上記の手順でカスタム ZIP として選択して適用できます。
...
**ログ表示**: 既定のウィンドウは `800 × 800`、ログ欄は 6 行分です。10 ポイントの文字に行間を追加しています。中日文では対応する等幅フォントを優先し、なければ各言語のローカルフォントを使用します。英語表示は等幅です。
