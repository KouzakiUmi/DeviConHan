...
# ================== Fuse 相关常量 ==================
FUSE_ENABLED_BYTE = b"\x31"  # Fuse 启用状态字节
FUSE_DISABLED_BYTE = b"\x30"  # Fuse 禁用状态字节
FUSE_VALIDATION_MIN_SIZE = 1024  # Fuse 校验最小文件大小
    ...
    def run_asar(self, action, src, dest, callback=None, unpack_pattern=None):
        ...
        try:
            ...
            proc = subprocess.run(
                ...
                timeout=get_config().get_int(
                    "main", "ASAR_OPERATION_TIMEOUT", fallback=300
                ),  # 超时时间从配置读取
            )
...
def _handle_backup_missing(core, asar_path, gui_app):
    ...
    if gui_app:
        ...
        result = _ask_yes_no(gui_app, title, msg)
        if not result:
            return (False, True)
...
def _verify_update_hash(core, asar_path, bak_path, patch_files, gui_app):
    ...
    if all_match_patch:
        ...
        title = T('title_success') or "Already Patched"
        msg = T('msg_already_patched') or "The game is already patched. No need to apply again."
        _show_info(gui_app, title, msg)
        return (False, False)
    else:
        ...
        if all_match_bak:
            ...
            title = T('title_third_party_patch_detected') or "Modified ASAR Detected"
            msg = T("msg_third_party_patch_detected") or "ASAR file has been modified by an unknown source or major Steam update.\nDo you want to rebuild backup and apply this patch?"
...
def _handle_both_exist(core, base_dir, asar_path, bak_path, gui_app):
    ...
    if not os.path.exists(meta_file):
        ...
        if not os.path.exists(info_file):
            ...
            if gui_app:
                result = _ask_yes_no(gui_app, T("title_no_patch_info"), T("msg_no_patch_info"))
                return (result, not result)
            return (True, False)
        else:
...
def batch_mode(args):
    ...
    asar = os.path.join(res, get_config().target_asar_name)
    bak = asar + ".bak"
    ...
    should_continue, cancel_or_error = handle_steam_update(core, base, bak, asar)
    ...
    if not os.path.exists(asar):
        logger.error(f"{get_config().target_asar_name} not found")
        return 1
    ...
    if args.auto:
        ...
        if not os.path.exists(bak):
            ...
            # 备份已存在，提醒用户这是重新打补丁
            logger.warning(
                f"Backup already exists: {bak} - this appears to be a re-patch operation"
            )

        # 解包
        temp = os.path.join(base, get_config().temp_patch_dir)
        ...
        try:
            ...
            # 应用补丁
            patch_zip = get_resource_path(get_config().patch_zip_name)
            patch_dir = get_resource_path(get_config().patch_dir_name)
            ...
            if os.path.exists(patch_zip):
                ...
                try:
                    ...
                    logger.error(
                        f"Failed to extract {get_config().patch_zip_name}: {patch_err}"
                    )
                    return 1
                ...
                logger.error(
                    "Patch data not found - cannot continue without patch files"
                )
                return 1
