...
保留安装/还原标签，但默认禁用该标签，无法进入第一页；在“开发者工具 → 配置管理”
勾选“启用补丁安装”后才可点击进入。若关闭时正在第一页，自动切回开发者工具页。
工具箱安装必须明确选择 ZIP，不会回退到默认补丁源；本地还原不要求选择补丁包。内置
补丁版默认开启，支持内置 ZIP / 目录或自定义 ZIP。两种模式分别保存
`preferences.patch_enabled_bundled` / `preferences.patch_enabled_toolbox`，避免共用用户配置
时互相覆盖。开关不影响批处理入口；GUI 操作进行中不允许切换。自定义 ZIP 选择仅在
本次会话保留，切换界面语言不会丢失选择。
...
GUI 主窗口默认 `800 × 800`，最小尺寸仍为 `800 × 680`，分别由
`WINDOW_DEFAULT_HEIGHT`、`WINDOW_MIN_WIDTH` 和 `WINDOW_MIN_HEIGHT` 控制。
`LOG_AREA_HEIGHT=6` 表示日志控件默认行数。日志字体为 10 磅，上下行距各 1 像素，
左右内边距 6 像素、上下内边距 4 像素；实际像素高度随字体和系统缩放变化。
`get_mono_font(10, available_families=...)` 使用主线程查询的 Tk 字体清单：中日文
优先选择对应的 CJK 等宽字体，缺少时使用本地字体，英文优先平台等宽字体。
这些显示设置用于主窗口内的日志区，不改变独立控制台字体或日志文件格式。
