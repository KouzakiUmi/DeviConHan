...
* **扫描与去重**：`scan_saves()` 会**同时扫描**老的游戏目录和新的备份目录。如果有同名备份，基于时间戳降序显示。
